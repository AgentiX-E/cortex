...
export type NaturalLanguageMemorySystemOptions = {
  ...
  /**
   * How the single-session path expands its hits into a prompt.
   *
   * `session` (default) admits a hit's whole session before spending budget on
   * `contextRadius` neighbours, which is what a question with a located answer
   * wants: the answer's surrounding turns carry the qualifier that disambiguates
   * it.
   *
   * `hits` admits each hit with its `contextRadius` neighbours but WITHOUT
   * session completion, and caps the total at `abstentionAdmissionBudget`. It
   * exists for the abstention path, where the expected answer is that NO
   ...
   * would also reset the query-expansion and answer caches and make the path
   ...
   * and no reasoning, and 10 of the 11 affected questions are flaky — the same
   * prompt, run again, produces the correct answer. Re-asking cannot lose: it
   ...
   * recovery. It fires on the bare form only: a reasoned abstention is a verdict
   * the engine respects, so re-asking it is noise.
   */
  enableAbstentionRetry?: boolean;
...
export class NaturalLanguageMemorySystem implements SessionAwareMemorySystem {
  ...
  private async retrieveTurns(
    ...
    sessions?: string[][],
    admissionMode: 'session' | 'hits' = 'session',
    ...
    const retrieved =
      hits.length === 0
        ? ''
        : (this.options.admissionMode ?? admissionMode) === 'hits'
          ? // Hits plus their `contextRadius` neighbours, WITHOUT session
            // completion and under a tighter ceiling. This is the abstention
            // path: its expected answer is that no evidence exists, so there is
            // no session to complete and completion can only append turns that
            // compete with the abstention. The ceiling is what actually does the
            // work -- see `abstentionAdmissionBudget`.
            expandContextWindowBounded(
              searchable,
              hits.map((h) => h.index),
              this.options.contextRadius ?? DEFAULT_CONTEXT_RADIUS,
              this.options.abstentionAdmissionBudget ?? DEFAULT_ABSTENTION_ADMISSION_BUDGET,
            )
          : expandContextWindowBySession(
              searchable,
              hits.map((h) => h.index),
  ...
  private async respondWith(
    ...
    /**
     * The second attempt's prompt IS the first prompt, and the retry goes
     * through the ordinary cache-first `complete`. Both halves matter and they
     * interact:
     *
     *   - because the prompt is identical, the cache key is the one the first
     *     attempt just wrote, so the re-ask resolves from memory and the
     *     provider is not billed for it. Under the rewritten-instruction
     *     design the retry had its OWN key, so every fire cost a real request
     *     (14 per run on `35162802298`) to learn nothing;
     *   - because the prompt is identical, the re-ask cannot differ from the
     *     first attempt at temperature 0, so the mechanism has no yield. That
     *     is the cost, it is measured, and it is recorded rather than papered
     *     over -- see `analysis/verdicts/p28-retry-redesign-verdict.md`.
     *
     * The alternative reading -- "the retry must bypass the cache or it reads
     * back the abstention it is escaping" -- was only ever true of a design
     * whose second prompt differed. With the prompt held equal, reading back
     * the first attempt IS the second attempt.
     */
    ...
    // same rule.
    //
    // The re-ask is byte-identical BY DECISION. It was rewritten to ask
    // something different in f39e7f9 and that rewrite was falsified by run
    // 35162802298: 14 fires, 0 recoveries, on a population where 10 of the 14
    // had their evidence in the context and 5 had it at top1Score 1.000. Across
    // two designs and 26 fires the yield is 0, so the wording is not the lever
    // and a third rewrite would be a guess. The mechanism stays wired and
    // measured; see the verdict's section 8 for whether to keep paying for it.
    ...
    if (critiquePass && !isAbstentionValue(raw, this.options.abstainToken)) {
      ...
      if (critique !== '') {
        ...
        const revised = await complete(revisionPrompt);
        if (
...
export function buildTemporalEventExtractionPrompt(
  ...
  const example = isOrdering
    ...
    : [
        ...
        '[2023/01/15] user: I went to the Ancient Civilizations exhibit yesterday.',
        'Question: How many days passed between my visit to MoMA and the Ancient Civilizations exhibit?',
        '{"events": [{"name": "visit Museum of Modern Art", "date": "2023/01/08"}, {"name": "visit Ancient Civilizations exhibit", "date": "yesterday", "turnDate": "2023/01/15"}]}',
      ];
  return [
    ...
    'An event date may be stated absolutely (the [YYYY/MM/DD] turn prefix) or relative to the turn that states it ("a month ago", "two weeks before", "3 days ago", "yesterday").',
    'When the turn states the event relative to its OWN date, report that relative time VERBATIM as "date" AND the turn\'s [YYYY/MM/DD] prefix as "turnDate". Do NOT convert it to an absolute date; exact date arithmetic converts it later using "turnDate" as the reference.',
    'When the turn states the event absolutely, report the [YYYY/MM/DD] turn prefix as "date" and omit "turnDate".',
    "If you cannot determine an event's date, omit that event entirely rather than guessing.",
    'Do NOT compute elapsed time, reorder events, or convert relative times to absolute dates. Just report each event name and its date (absolute or verbatim relative, with "turnDate" when relative).',
 ...
 *
 * A second, distinct abstention defect was measured afterwards: across 968 MR
 * decisions the model returned a BARE `UNANSWERABLE` 35 times — the entire
 * response, under 40 characters, with no `Step 1` list and no `Step 2`
 * reasoning. Those are not verdicts, they are skips: the model never wrote down
 * what it read. A prohibition phrased as a CONDITION ("only if there is no
 * relevant information") cannot catch them, because the model believes it
 * satisfies the condition at the moment it gives up. The clause below therefore
 * constrains the ORDER of the work instead: no token until the list is written.
 * `detectBareAbstention` and the bounded retry in `respondWith` enforce the same
 * contract from the outside, since the prompt alone is advisory.
 */
export function buildAggregationQaPrompt(
  ...
  return [
    ...
    `Do NOT write "${abstainToken}" without first writing your Step 1 list. Confirm that you completed Step 1 by listing each candidate item you considered, including the ones you decided to exclude and why.`,
    `A bare "${abstainToken}" with no Step 1 list is not a valid response.`,
    '',
    'Context:',
    context,
    '',
 ...
 *
 * Measured on LongMemEval-S, 35 of 968 multi-session decisions are exactly this
 * — the token alone, 12 characters, no `Step 1` list and no `Step 2` reasoning.
 * They are a distinct defect from a reasoned abstention: the reasoned form is a
